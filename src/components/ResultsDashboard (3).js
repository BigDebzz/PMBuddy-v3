      ...
      <div style={s.wrap}>
        ...
        <div style={s.header}>
          <div>
            <div style={s.breadcrumb}>
              <span style={{ color: B, fontWeight: 700 }}>{config.label}</span>
        ...
        <div style={{ ...s.scoreBanner, borderTop: `3px solid ${B}` }}>
          ...
          <div style={s.scoreRight}>
            <span style={s.countStrength}>{analysis.insights?.length || 0} strengths found</span>
            <span style={s.countRisk}>{analysis.challenges?.length || 0} issues to fix</span>
          </div>
        </div>
...
function MethodologyTab({ analysis }) {
  ...
  return (
    <div>
      ...
      <div style={{ ...s.methodCard, borderTop: `3px solid ${B}` }}>
        <h3 style={{ ...s.methodName, color: B }}>{m.name}</h3>
...
function FeedbackForm({ mode }) {
  ...
  const handleSubmit = async () => {
    if (rating === 0) return;
    setLoading(true);
    await supabase.from('feedback').insert({ mode, rating, useful_text: useful, missing_text: missing, email: email || null });
    setLoading(false);
    setSubmitted(true);
  };
