...
export default function LandingScreen({ onSelectMode, onLogin, onSignup, onDashboard, user }) {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(16px)', transition: 'all 0.6s ease 0.1s' }}>
      ...
      <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(16px)', transition: 'all 0.6s ease 0.2s' }}>
      ...
      <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(16px)', transition: 'all 0.6s ease 0.4s' }}>
      ...
      <div style={{ opacity: visible ? 1 : 0, transition: 'all 0.6s ease 0.5s' }}>
        <div style={s.cta}>
          <div style={s.ctaInner}>
            ...
            <div style={s.ctaBtns}>
              {user ? (
                <button style={s.ctaPrimaryBtn} onClick={onDashboard}>Go to my projects</button>
              ) : (
                <>
...
function ModeCard({ title, badge, time, desc, points, onClick }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div
      style={{ ...s.modeCard, borderColor: hovered ? B : '#E5E7EB', boxShadow: hovered ? `0 12px 40px ${B}20` : '0 2px 8px rgba(0,0,0,0.06)', transform: hovered ? 'translateY(-3px)' : 'translateY(0)' }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <div style={s.modeCardTop}>
        <p style={s.modeBadge}>{badge}</p>
      ...
      <button style={s.modeBtn} onClick={onClick}>
...
const s = {
  ...
  hero: { background: BL, padding: '80px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  ...
  heroSub: { fontSize: 16, color: '#9CA3AF', lineHeight: 1.8, marginBottom: 32, maxWidth: 520, margin: '0 auto 32px' },
  heroBtns: { display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16, justifyContent: 'center' },
  ...
  modeBadge: { display: 'inline-block', fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 100, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8, background: B + '12', color: B },
  ...
  modeDot: { width: 6, height: 6, borderRadius: '50%', flexShrink: 0, background: B },
  modeBtn: { width: '100%', color: WH, border: 'none', borderRadius: 10, padding: '13px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', background: B },
