    ...
    <div>
      ...
      {docs.map(doc => (
        <div key={doc.id} style={s.docCard}>
          ...
          <div style={s.docBtns}>
            <button
              style={{ ...s.downloadBtn, opacity: generating === doc.id ? 0.7 : 1 }}
              onClick={() => downloadHTML(doc.id)}
              disabled={!!generating}
            >
              {generating === doc.id ? 'Generating...' : 'Download PDF'}
            </button>
            <button
              style={{ ...s.downloadBtnOutline, opacity: generating === doc.id + '_word' ? 0.7 : 1 }}
